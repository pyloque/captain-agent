...
public class DiscoveryService {
	 ...
	 * @return
	 */
	private String keyForVersion(String name) {
	 ...
	 * @return
	 */
	private String keyForSet(String name) {
		return "service_set_" + name;
	}

	/* All Service names */
	private final static String nameKeys = "service_names";
	...
	public void keepService(ServiceItem item) {
		...
		this.redis.execute(jedis -> {
			jedis.sadd(nameKeys, item.name());
			count.set(jedis.zadd(keyForSet(item.name()), now + item.ttl(), item.key()));
		});
		if (count.value() > 0) {
			this.redis.transaction(pipe -> {
				pipe.incr(keyForVersion(item.name()));
				pipe.incr(globalVersionKey);
			});
		}
	}

	public void cancelService(ServiceItem item) {
		List<String> watchedKeys = new ArrayList<String>(1);
		watchedKeys.add(keyForVersion(item.name()));
		Holder<Double> score = new Holder<Double>();
		this.redis.execute(jedis -> {
			score.set(jedis.zscore(keyForSet(item.name()), item.key()));
		});
		if (score.value() == null) {
			return;
		}
		this.redis.transaction(pipe -> {
			pipe.zrem(keyForSet(item.name()), item.key());
			pipe.incr(keyForVersion(item.name()));
			pipe.incr(globalVersionKey);
		});
	}

	public long getServiceVersion(String name) {
	...
	public long getServiceGlobalVersion() {
		Holder<Long> version = new Holder<Long>();
		this.redis.execute(jedis -> {
	...
	public Map<String, Long> getMultiServiceVersions(String[] names) {
	...
	public Map<String, Integer> getMultiServiceLens(String[] names) {
